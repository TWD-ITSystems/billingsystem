...
namespace BillingSystem
{
    public partial class formDataRepair : Form
    {
        ...
        private void transferReadings()
        {
            ...
            int countPresent = 0;
            string str = "";
            str = string.Format(@"D:\repair\{0}-accts_present_in_sql.txt", txtZoneBook.Text);
        ...
        {
            int counter = 0;
            string logfile = string.Format("D:\\repair\\repairzone-{0}.txt", textBox1.Text);
            using (dbCommercialEntities dbcontext = new dbCommercialEntities())
            {
               ...
               //             global_func.saveLogFile(logfile, string.Format("{0}-{1} new:{2} old:{3}", dsRec.md_zone, dsRec.md_acctno, dsRec.md_billno, oldbill));
                        ...
                        {
                            dsRec.md_billno = dsRec.md_remark;
                            dsRec.md_billamount = globFunc.processCurrent(masterid, vol).ToString();
                            dsRec.md_billmonth = "05-2014";
                            globFunc.saveLogFile(@"D:\repairzone.txt", dsRec.md_acctno);
                        }
                         */
                        // end slight repair
                    }
        ...
        private void btnUpdateBill_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                {
                    conn.Open();
                    string query = string.Format("Select [NAME],[ACCTNO],[ZONE],[CURRENT],[CARDNO],[ARREARS],[PRESRDG],[REF],[DATEPRES],[VOLUME],[PREVRDG],[PENALTY],[AVE],[OTHERS],[DATEPREV] from [{0}]", fileName);
                    using (OleDbDataAdapter myAdapter = new OleDbDataAdapter(query, conn))
                    {
                        ...
                        foreach (var drow in dRows)
                        {
                          ...
                          //  if (!temp.Equals("018632")) continue;
                            ...
                            rData.current = Convert.ToDecimal(drow["CURRENT"].ToString()) - 5;
                            ...
                            rData.rdg_prev = Convert.ToInt32(drow["PREVRDG"].ToString());
                            ...
                            rData.encoder = global_func.getMtrReaderByZone(zone);
        ...
        private void UpdateMasterAndAccounts(ReadLog rData)
        {
            using (dbCommercialEntities dbcontext = new dbCommercialEntities())
            {
                ...
                {
                    masterAcct.bill_balance = masterAcct.bill_balance + rData.current + rData.mtr_mtn;
                    masterAcct.bill_current = rData.current;
                ...
                if (rData.inst_fee > 0)
                {
                    ...
                    ledgerData.master_id = rData.master_id;
                    //  ledgerData.billing_id = billingID;
                    ledgerData.amount = rData.inst_fee;
                    ledgerData.balance = masterAcct.bill_balance;
                    dbcontext.AddTotbLedgers(ledgerData);
                }
                    ...
                    using (OleDbDataAdapter myAdapter = new OleDbDataAdapter(query, conn))
                    {
                        ...
                        using (dbCommercialEntities dbcontext = new dbCommercialEntities())
                        {
                            foreach (var drow in dRows)
                            {
                              ...
                              //if (!acctno.Equals("028090")) continue;
                                ...
                                if (sdrcode.Equals("IF2"))
                                {
                                    ...
                                    if (sdramt > 0)
                                    {
                                        ...
                                        if (sdramt == collected)
                                        {
                                            ...
                                            ledgerData.trans_date = Convert.ToDateTime(drow["DATE"].ToString());
                                            ledgerData.symbol = "";
                ...
                using (OleDbConnection conn = new OleDbConnection(connString))
                {
                    ...
                    using (OleDbDataAdapter myAdapter = new OleDbDataAdapter(query, conn))
                    {
                        ...
                        foreach (var rData in dRows)
                        {
                            ...
                            decimal balance = Convert.ToDecimal(rData.BALANCE);
                            int masterid = 0;
                            int meterid = 0;
                            ...
                            using (dbCommercialEntities dbcontext = new dbCommercialEntities())
                            {
                                ...
                                {
                                   // continue;    //uncomment to enable add only new accounts..
                                    if (salardaCounter == 1)
                                    {
                                        salardaCounter = 0;
                                        continue;
